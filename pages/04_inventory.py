...
st.info(f"📍 Mostrando datos exclusivos de la planta: **{st.session_state.site_name}**")
...
# --- PESTAÑA A: DIRECTORIO DE ACTIVOS ---
with tab_dir:
    st.markdown(f"#### {t('inventory', 'tab_dir')}")
    
    with st.spinner("Cargando base de datos..."):
        try:
            # 🛡️ CONSULTA MULTI-TENANT: Siempre filtramos por site_id
            ...
            if not df_assets.empty:
                ...
                # Formato visual
                df_mostrar.columns = ["ID Activo", "Categoría", "Clasificación", "Ubicación", "Estatus Operativo"]
                df_mostrar["Estatus Operativo"] = df_mostrar["Estatus Operativo"].apply(
                    lambda x: f"🟢 {x}" if x == 'ACTIVE' else f"🔴 {x}"
                )
...
# --- PESTAÑA B: REGISTRAR NUEVO ACTIVO ---
with tab_add:
    ...
    with st.form("form_add_asset", clear_on_submit=True):
        ...
        if st.form_submit_button(t("inventory", "btn_save"), type="primary", use_container_width=True):
            if not nuevo_id.strip():
                st.error("El ID del activo no puede estar vacío.")
            else:
                with st.spinner("Registrando..."):
                    try:
                        # 🛡️ INSERCIÓN MULTI-TENANT: Forzamos la inyección del site_id del usuario
                        payload = {
                            "site_id": site_id,
...
# --- PESTAÑA C: DAR DE BAJA ---
with tab_retire:
    st.markdown(f"#### {t('inventory', 'tab_retire')}")
    st.caption("Cambia el estatus de un activo a inactivo sin eliminar su historial de auditorías.")
    
    if 'df_assets' in locals() and not df_assets.empty:
        # Solo mostrar equipos que están activos actualmente
        ...
        if activos_disponibles:
            with st.form("form_retire_asset"):
                ...
                if st.form_submit_button("🗑️ Desactivar Activo", type="secondary"):
                    with st.spinner("Actualizando estatus..."):
                        try:
                            # 🛡️ UPDATE SEGURO: Doble candado con site_id y custom_id
