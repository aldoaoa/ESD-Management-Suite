    ...
    with col1:
        ...
        with st.form("form_add_location", clear_on_submit=True):
            ...
            if st.form_submit_button(t("settings", "btn_save"), type="primary", use_container_width=True):
                if loc_name.strip():
                        if not site_id:
                            st.error("No site selected. Please choose a site before adding a location.")
                        else:
                            try:
    ...
    with col2:
        if site_id:
            try:
                ...
                st.error("No se pueden cargar las ubicaciones (DB error).")
        else:
            st.info("Seleccione un site válido para ver las ubicaciones.")
    ...
    with col_e1:
        ...
        with st.form("form_add_eq", clear_on_submit=True):
            ...
            if st.form_submit_button(t("settings", "btn_save"), type="primary", use_container_width=True):
                if eq_id.strip():
                    if not site_id:
                        st.error("No site selected. Please choose a site before adding equipment.")
                    else:
                        try:
    ...
    with col_e2:
        if site_id:
            try:
                ...
                st.error("No se pueden cargar equipos (DB error).")
        else:
            st.info("Seleccione un site válido para ver los equipos.")
